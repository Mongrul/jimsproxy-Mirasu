...
public sealed class GameSessionData
{
    ...
    // JimsProxy (proficiency-synth 2026-05-10): last-emitted synthesized
    // proficiency masks for the local player. Vanilla 1.12 servers skip the
    // at-login SMSG_SET_PROFICIENCY broadcast (Twinstar Kronos confirmed
    // silent in tester JSONL); the modern client falls back to "no
    // restrictions" rendering all off-class armor / weapons in white.
    // The proxy synthesizes a proficiency mask from class baseline OR'd
    // with proficiency-granting spells in the known-spells set, and
    // re-emits whenever the recomputed mask differs (covers trainer
    // additions for Plate / Mail / Polearms etc. that arrive via
    // SMSG_LEARNED_SPELL mid-session). uint.MaxValue = "never emitted"
